...
EntityButtonStart = EntityButton.extend({
	size: { x: 181, y: 60 },
	animSheet: new ig.AnimationSheet('media/img/'+_LANG_STRING+'button-start.png', 181, 60),
	update: function() {
		if(ig.input.pressed('click') && this.inFocus()) {
			if(ig.Sound.enabled) {
				this.sound.play();
...
EntityButtonBack = EntityButton.extend({
	size: { x: 186, y: 58 },
	animSheet: new ig.AnimationSheet('media/img/'+_LANG_STRING+'button-back.png', 186, 58),
	update: function() {
		if(ig.input.pressed('click') && this.inFocus()) {
			if(ig.Sound.enabled)
				this.sound.play();
...
EntityButtonContinue = EntityButton.extend({
	size: { x: 132, y: 43 },
	animSheet: new ig.AnimationSheet('media/img/'+_LANG_STRING+'button-continue.png', 132, 43),
	update: function() {
	    if(ig.input.pressed('click') && this.inFocus()) {
			if(ig.Sound.enabled)
				this.sound.play();
...
EntityButtonRestart = EntityButton.extend({
	size: { x: 186, y: 58 },
	animSheet: new ig.AnimationSheet('media/img/'+_LANG_STRING+'button-restart.png', 186, 58),
	update: function() {
	    if(ig.input.pressed('click') && this.inFocus()) {
			if(ig.Sound.enabled)
				this.sound.play();
...
EntityUIShield = ig.Entity.extend({
	...
	maxVel: { x: 0, y: 0 },
	animSheet: new ig.AnimationSheet('media/img/'+_LANG_STRING+'ui-shield.png', _LANG_SIZES[_LANG_CODE]['shield'].x, _LANG_SIZES[_LANG_CODE]['shield'].y),
	type: ig.Entity.TYPE.NONE,
	init: function(x, y, settings) {
...
EntityUIScore = ig.Entity.extend({
	...
	maxVel: { x: 0, y: 0 },
	animSheet: new ig.AnimationSheet('media/img/'+_LANG_STRING+'ui-score.png', _LANG_SIZES[_LANG_CODE]['score'].x, _LANG_SIZES[_LANG_CODE]['score'].y),
	type: ig.Entity.TYPE.NONE,
	init: function(x, y, settings) {
		this.addAnim('idle', 0.1, [0]);
		this.parent(x, y, settings);
...
RogersGame = ig.Game.extend({
	...
	gameOverScreenAsteroid: new ig.Image('media/img/'+_LANG_STRING+'screen-gameover-asteroid.png'),
	gameOverScreenMine: new ig.Image('media/img/'+_LANG_STRING+'screen-gameover-mine.png'),
	gameOverScreenOffscreen: new ig.Image('media/img/'+_LANG_STRING+'screen-gameover-offscreen.png'),
	gameOverScore: new ig.Image('media/img/'+_LANG_STRING+'gameover-score.png'),
	gameOverBest: new ig.Image('media/img/'+_LANG_STRING+'gameover-best.png'),
	gameOverHighscore: new ig.Image('media/img/'+_LANG_STRING+'gameover-highscore.png'),
	gamePausedScreen: new ig.Image('media/img/'+_LANG_STRING+'screen-pause.png'),
	gameCompletedScreen: new ig.Image('media/img/'+_LANG_STRING+'screen-gamecompleted.png'),
...
StartScreen = ig.Game.extend({
	...
	bestScore: new ig.Image('media/img/'+_LANG_STRING+'gameover-best.png'),
	storage: new ig.Storage(),
	counter: 0,
	parallax: null,
	init: function() {
...
StoryScreen = ig.Game.extend({
	story: new ig.Image('media/img/'+_LANG_STRING+'screen-story.png'),
	buttonContinue: new ig.Image('media/img/'+_LANG_STRING+'button-continue.png'),
	init: function() {
		ig.input.bind(ig.KEY.SPACE, 'start');
		ig.input.bind(ig.KEY.ENTER, 'start');
...
HowToScreen = ig.Game.extend({
	howTo: new ig.Image('media/img/'+_LANG_STRING+'screen-howto.png'),
	buttonContinue: new ig.Image('media/img/'+_LANG_STRING+'button-continue.png'),
	init: function() {
		ig.input.bind(ig.KEY.SPACE, 'start');
		ig.input.bind(ig.KEY.ENTER, 'start');
