...
if(ig.Sound.enabled)
this.sound.play();this.kill();},kill:function(){if(this.pos.x+50>0){if(ig.game.BtnBomb&&ig.game.BtnBomb.bombingInProgress){ig.game.spawnEntity(EntityAsteroidExplosion,this.pos.x-42,this.pos.y-42);ig.game.achievementsCounter.asteroids++;console.log('asteroid destroyed by bombing, asteroids counter: '+ig.game.achievementsCounter.asteroids);if(ig.game.achievementsCounter.asteroids==50){(new Clay.Achievement({id:"rogers-destroyer"})).award();}}
...
this.parent();}});EntityButton=ig.Entity.extend({sound:new ig.Sound('media/audio/button-click.*'),init:function(x,y,settings){this.addAnim('idle',0.1,[0]);this.parent(x,y,settings);},inFocus:function(){return((this.pos.x<=(ig.input.mouse.x+ig.game.screen.x))&&((ig.input.mouse.x+ig.game.screen.x)<=this.pos.x+this.size.x)&&(this.pos.y<=(ig.input.mouse.y+ig.game.screen.y))&&((ig.input.mouse.y+ig.game.screen.y)<=this.pos.y+this.size.y));}});EntityButtonStart=EntityButton.extend({size:{x:183,y:60},animSheet:new ig.AnimationSheet('media/img/button-start.png',183,60),update:function(){if(ig.input.pressed('click')&&this.inFocus()){if(ig.Sound.enabled){this.sound.play();}
...
this.sound.play();for(var i=0;i<ig.game.entities.length;i++){if(ig.game.entities[i].type==ig.Entity.TYPE.B){if(ig.game.entities[i].direction){ig.game.achievementsCounter.mines++;console.log('mine destroyed while bombing, mines counter: '+ig.game.achievementsCounter.mines);if(ig.game.achievementsCounter.mines==10){(new Clay.Achievement({id:"rogers-minesweeper"})).award();}}
ig.game.entities[i].kill();}}
this.currentAnim=this.anims['inactive'];ig.game.activateBombing=false;ig.game.BtnBomb.bombingInProgress=false;ig.game.achievementsCounter.bombs++;console.log('bomb used, bombs counter: '+ig.game.achievementsCounter.bombs);if(ig.game.achievementsCounter.bombs==10){(new Clay.Achievement({id:"rogers-bomberman"})).award();}}}}});EntityButtonAudio=EntityButton.extend({size:{x:42,y:44},animSheet:new ig.AnimationSheet('media/img/button-audio.png',42,44),init:function(x,y,settings){this.addAnim('true',0.1,[0]);this.addAnim('false',0.1,[1]);this.parent(x,y,settings);},update:function(){if(ig.input.pressed('click')&&this.inFocus()){if(!ig.ua.iOS){ig.Sound.enabled=!ig.Sound.enabled;if(ig.game.storage.isCapable())
...
return;}
this.parent();},draw:function(){this.pos.x=ig.game.screen.x;this.parent();}});RogersGame=ig.Game.extend({clearColor:null,gravity:350,player:null,gameOver:false,gameOverType:null,gamePaused:false,gameCompleted:false,maxScore:20000,score:0,speed:1.5,seconds:null,secondsTimer:null,font:new ig.Font('media/telemarines.font.png'),background:new ig.Image('media/img/deepspace.png'),stars:new ig.Image('media/img/stars.png'),gameOverScreenAsteroid:new ig.Image('media/img/screen-gameover-asteroid.png'),gameOverScreenMine:new ig.Image('media/img/screen-gameover-mine.png'),gameOverScreenOffscreen:new ig.Image('media/img/screen-gameover-offscreen.png'),gameOverScore:new ig.Image('media/img/gameover-score.png'),gameOverBest:new ig.Image('media/img/gameover-best.png'),gameOverHighscore:new ig.Image('media/img/gameover-highscore.png'),gamePausedScreen:new ig.Image('media/img/screen-pause.png'),gameCompletedScreen:new ig.Image('media/img/screen-gamecompleted.png'),storage:new ig.Storage(),parallax:null,bombsCount:0,activateBombing:false,achievementsCounter:{asteroids:0,bombs:0,mines:0,stars:0},init:function(){ig.input.bind(ig.KEY.UP_ARROW,'up');ig.input.bind(ig.KEY.MOUSE1,'click');ig.input.bind(ig.KEY.ENTER,'ok');ig.input.bind(ig.KEY.P,'pause');ig.input.bind(ig.KEY.B,'bomb');ig.input.bind(ig.KEY.C,'cheat code');this.secondsTimer=new ig.Timer(1);this.parallax=new Parallax();this.parallax.add(this.background.path,{distance:5,y:0});this.parallax.add(this.stars.path,{distance:2,y:0});this.player=this.spawnEntity(EntityPlayer,40,72);this.UIShield=this.spawnEntity(EntityUIShield,55,5);this.UIScore=this.spawnEntity(EntityUIScore,ig.system.width-205-47-5,5);this.BtnPause=this.spawnEntity(EntityButtonPause,ig.system.width-47-47-5,5);this.BtnAudio=this.spawnEntity(EntityButtonAudio,ig.system.width-47,5);this.BtnBomb=this.spawnEntity(EntityButtonBomb,5,5);if(ig.Sound.enabled){this.BtnAudio.currentAnim=this.BtnAudio.anims['true'];}
else{this.BtnAudio.currentAnim=this.BtnAudio.anims['false'];}
this.newItemTable();ig.game.player.vel.y=0;this.oldHighscore=ig.game.storage.get('rogers-highscore');this.settingTheScore=false;this.rogersNewcomer=false;this.rogersScoremaster=false;this.rogersCaptain=false;this.rogersScore=false;this.rogersLeaderboard=new Clay.Leaderboard({id:1679});},placeEntity:function(entity){var x=ig.system.width+50;var y=Math.random()*ig.system.height;var item=this.spawnEntity(entity,x,y);item.speed=ig.game.speed+Math.random()*ig.game.speed/2-Math.random()*ig.game.speed/2;if(y>ig.system.height-item.size.y)
...
this.shuffleArray(this.entityItemTable);},update:function(){if(this.score>=this.maxScore){if(!this.rogersCaptain){(new Clay.Achievement({id:"rogers-captain"})).award();this.rogersLeaderboard.post({score:20000},function(response){});this.rogersCaptain=true;}
if(!this.BtnCompleted){this.BtnCompleted=this.spawnEntity(EntityButtonBack,ig.system.width-206-10,ig.system.height-58-10);}
...
else if(this.gameOver){if(!this.rogersNewcomer){(new Clay.Achievement({id:"rogers-newcomer"})).award();this.rogersNewcomer=true;}
if(!this.BtnRestart)
this.BtnRestart=this.spawnEntity(EntityButtonRestart,10,ig.system.height-58-10);if(!this.BtnBack)
this.BtnBack=this.spawnEntity(EntityButtonBack,ig.system.width-206-10,ig.system.height-58-10);this.BtnRestart.update();this.BtnBack.update();this.BtnAudio.update();var newHighscore=(this.score).floor();if(!this.rogersScoremaster){if(newHighscore>=1000){(new Clay.Achievement({id:"rogers-scoremaster"})).award();this.rogersScoremaster=true;}}
if(!this.rogersScore){this.rogersLeaderboard.post({score:newHighscore},function(response){console.log('response!');});this.rogersScore=true;}
if(newHighscore>this.oldHighscore&&!this.settingTheScore){ig.game.storage.setHighest('rogers-highscore',newHighscore);this.settingTheScore=true;}}
...
this.BtnBack.draw();}
else if(this.gameOver){var x=(ig.system.width-254)/2;var y=((ig.system.height-147)/2)-30;switch(this.gameOverType){case'offscreen':{this.gameOverScreenOffscreen.draw(x,y);break;}
...
this.counter++;this.parallax.move(40);this.parent();},draw:function(){this.parent();this.parallax.draw();this.BtnStart.draw();this.logoBlackmoon.draw(10,10);this.logoEnclave.draw(55,10);this.BtnFacebook.draw();this.BtnAudio.draw();var range=((this.counter/3)%60)-30;var delta=(range>0)?1:-1;var tick=range*delta;this.ship.draw(-30,ig.system.height-this.ship.height+tick+10);this.logo.draw((ig.system.width-this.logo.width)/2-20,10);this.title.draw((ig.system.width-this.title.width+160)/2,100);this.bestScore.draw(10,ig.system.height-10-50);this.font.draw(this.oldHighscore,70,ig.system.height-28,ig.Font.ALIGN.CENTER);}});StoryScreen=ig.Game.extend({story:new ig.Image('media/img/screen-story.png'),buttonContinue:new ig.Image('media/img/button-continue.png'),init:function(){ig.input.bind(ig.KEY.SPACE,'start');ig.input.bind(ig.KEY.ENTER,'start');ig.input.bind(ig.KEY.MOUSE1,'start');},update:function(){if(ig.input.pressed('start')){ig.system.setGame(HowToScreen);}
...
ig.CanvasCSS3Scaling=new CanvasCSS3Scaling();ig.CanvasCSS3Scaling.init();ig.main('#canvas',StartScreen,30,480,320,1,ig.ImpactSplashLoader);});