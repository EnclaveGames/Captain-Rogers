...
this.counter++;this.parallax.move(40);this.parent();},draw:function(){this.parent();this.parallax.draw();this.BtnStart.draw();this.logoBlackmoon.draw(10,10);this.logoEnclave.draw(55,10);this.BtnFacebook.draw();this.BtnAudio.draw();var range=((this.counter/3)%60)-30;var delta=(range>0)?1:-1;var tick=range*delta;this.ship.draw(-30,ig.system.height-this.ship.height+tick+10);this.logo.draw((ig.system.width-this.logo.width)/2,10);this.title.draw((ig.system.width-this.title.width+160)/2,100);this.bestScore.draw(10,ig.system.height-10-50);this.font.draw(this.oldHighscore,70,ig.system.height-28,ig.Font.ALIGN.CENTER);}});StoryScreen=ig.Game.extend({story:new ig.Image('media/img/screen-story.png'),buttonContinue:new ig.Image('media/img/button-continue.png'),init:function(){ig.input.bind(ig.KEY.SPACE,'start');ig.input.bind(ig.KEY.ENTER,'start');ig.input.bind(ig.KEY.MOUSE1,'start');},update:function(){if(ig.input.pressed('start')){ig.system.setGame(HowToScreen);}
...
function handleVisibilityChange(){if(document[hidden]){if(ig&&ig.music){console.log("hidden");ig.music.pause();}}else{if(ig&&ig.music){console.log("visible");ig.music.play();}}}
if(typeof document.addEventListener==="undefined"||typeof hidden==="undefined"){}else{document.addEventListener(visibilityChange,handleVisibilityChange,false);}
...
window.addEventListener("orientationchange",checkOrientation);ig.CanvasCSS3Scaling=new CanvasCSS3Scaling();ig.CanvasCSS3Scaling.init();ig.main('#canvas',StartScreen,30,480,320,1,ig.ImpactSplashLoader);});