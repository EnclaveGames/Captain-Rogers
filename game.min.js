...
ig.baked=true;ig.module('plugins.canvas-css3-scaling').defines(function(){FullscreenHelper=ig.Class.extend({offset:null,scale:null,init:function(){this.element=document.getElementById('game');this.canvas=this.element.firstElementChild;this.content=[this.canvas.width,this.canvas.height];this.element.addEventListener('mousemove',this,true);window.addEventListener('resize',this,false);this.reflow();},reflow:function(){var browser=[window.innerWidth,window.innerHeight];var scale=this.scale=Math.min(browser[0]/this.content[0],browser[1]/this.content[1]);var size=[this.content[0]*scale,this.content[1]*scale];var offset=this.offset=[(browser[0]-size[0])/2,(browser[1]-size[1])/2];var rule="translate("+offset[0]+"px, "+offset[1]+"px) scale("+scale+")";this.element.style.transform=rule;this.element.style.webkitTransform=rule;},handleEvent:function(evt){if(evt.type=='resize'){this.reflow();}}});});

// lib/impact/input.js
ig.baked=true;ig.module('impact.input').requires('plugins.canvas-css3-scaling').defines(function(){"use strict";ig.KEY={'MOUSE1':-1,'MOUSE2':-3,'MWHEEL_UP':-4,'MWHEEL_DOWN':-5,'BACKSPACE':8,'TAB':9,'ENTER':13,'PAUSE':19,'CAPS':20,'ESC':27,'SPACE':32,'PAGE_UP':33,'PAGE_DOWN':34,'END':35,'HOME':36,'LEFT_ARROW':37,'UP_ARROW':38,'RIGHT_ARROW':39,'DOWN_ARROW':40,'INSERT':45,'DELETE':46,'_0':48,'_1':49,'_2':50,'_3':51,'_4':52,'_5':53,'_6':54,'_7':55,'_8':56,'_9':57,'A':65,'B':66,'C':67,'D':68,'E':69,'F':70,'G':71,'H':72,'I':73,'J':74,'K':75,'L':76,'M':77,'N':78,'O':79,'P':80,'Q':81,'R':82,'S':83,'T':84,'U':85,'V':86,'W':87,'X':88,'Y':89,'Z':90,'NUMPAD_0':96,'NUMPAD_1':97,'NUMPAD_2':98,'NUMPAD_3':99,'NUMPAD_4':100,'NUMPAD_5':101,'NUMPAD_6':102,'NUMPAD_7':103,'NUMPAD_8':104,'NUMPAD_9':105,'MULTIPLY':106,'ADD':107,'SUBSTRACT':109,'DECIMAL':110,'DIVIDE':111,'F1':112,'F2':113,'F3':114,'F4':115,'F5':116,'F6':117,'F7':118,'F8':119,'F9':120,'F10':121,'F11':122,'F12':123,'SHIFT':16,'CTRL':17,'ALT':18,'PLUS':187,'COMMA':188,'MINUS':189,'PERIOD':190};ig.Input=ig.Class.extend({bindings:{},actions:{},presses:{},locks:{},delayedKeyup:{},isUsingMouse:false,isUsingKeyboard:false,isUsingAccelerometer:false,mouse:{x:0,y:0},accel:{x:0,y:0,z:0},initMouse:function(){if(this.isUsingMouse){return;}
...
var ev=event.touches?event.touches[0]:event;this.mouse.x=(ev.pageX-ig.FullscreenHelper.offset[0])/ig.FullscreenHelper.scale;this.mouse.y=(ev.pageY-ig.FullscreenHelper.offset[1])/ig.FullscreenHelper.scale;},contextmenu:function(event){if(this.bindings[ig.KEY.MOUSE2]){event.stopPropagation();event.preventDefault();}},keydown:function(event){var tag=event.target.tagName;if(tag=='INPUT'||tag=='TEXTAREA'){return;}
...
if(ig.Sound.enabled)
this.sound.play();this.kill();},kill:function(){if(this.pos.x+50>0){ig.game.spawnEntity(EntityAsteroidExplosion,ig.game.player.pos.x,ig.game.player.pos.y-40);}
this.parent();}});EntityAsteroidExplosion=ig.Entity.extend({lifetime:0.3,size:{x:1,y:1},maxVel:{x:0,y:0},animSheet:new ig.AnimationSheet('media/img/asteroid-explosion.png',93,96),type:ig.Entity.TYPE.NONE,init:function(x,y,settings){this.addAnim('hit',0.03,[9,8,7,6,5,4,3,2,1,0]);this.parent(x,y,settings);this.idleTimer=new ig.Timer();},update:function(){if(this.idleTimer.delta()>this.lifetime){this.kill();return;}
this.parent();}});EntityButton=ig.Entity.extend({sound:new ig.Sound('media/audio/button-click.*'),init:function(x,y,settings){this.addAnim('idle',0.1,[0]);this.addAnim('hover',0.1,[1]);this.parent(x,y,settings);},inFocus:function(){return((this.pos.x<=(ig.input.mouse.x+ig.game.screen.x))&&((ig.input.mouse.x+ig.game.screen.x)<=this.pos.x+this.size.x)&&(this.pos.y<=(ig.input.mouse.y+ig.game.screen.y))&&((ig.input.mouse.y+ig.game.screen.y)<=this.pos.y+this.size.y));}});EntityButtonStart=EntityButton.extend({size:{x:183,y:60},animSheet:new ig.AnimationSheet('media/img/button-start.png',183,60),update:function(){if(ig.input.pressed('click')&&this.inFocus()){if(ig.Sound.enabled){this.sound.play();}
...
return;}
this.parent();},draw:function(){this.pos.x=ig.game.screen.x;this.parent();}});RogersGame=ig.Game.extend({clearColor:null,gravity:350,player:null,gameOver:false,gameOverType:null,gamePaused:false,gameCompleted:false,maxScore:20000,score:0,speed:1.5,seconds:null,secondsTimer:null,font:new ig.Font('media/telemarines.font.png'),background:new ig.Image('media/img/deepspace.png'),stars:new ig.Image('media/img/stars.png'),gameOverScreenAsteroid:new ig.Image('media/img/screen-gameover-asteroid.png'),gameOverScreenMine:new ig.Image('media/img/screen-gameover-mine.png'),gameOverScreenOffscreen:new ig.Image('media/img/screen-gameover-offscreen.png'),gamePausedScreen:new ig.Image('media/img/screen-pause.png'),gameCompletedScreen:new ig.Image('media/img/screen-gamecompleted.png'),storage:new ig.Storage(),parallax:null,init:function(){ig.input.bind(ig.KEY.UP_ARROW,'up');ig.input.bind(ig.KEY.MOUSE1,'click');ig.input.bind(ig.KEY.ENTER,'ok');ig.input.bind(ig.KEY.P,'p');this.secondsTimer=new ig.Timer(1);this.parallax=new Parallax();this.parallax.add(this.background.path,{distance:5,y:0});this.parallax.add(this.stars.path,{distance:2,y:0});this.player=this.spawnEntity(EntityPlayer,40,72);this.UIShield=this.spawnEntity(EntityUIShield,5,2);this.UIScore=this.spawnEntity(EntityUIScore,ig.system.width-205-47-5,5);this.BtnPause=this.spawnEntity(EntityButtonPause,ig.system.width-47-47-5,5);this.BtnAudio=this.spawnEntity(EntityButtonAudio,ig.system.width-47,5);if(ig.Sound.enabled){this.BtnAudio.currentAnim=this.BtnAudio.anims['true'];}
...
else{this.speed+=0.0005;this.score+=ig.system.tick*this.speed*5;for(var i=0;i<this.entities.length;i++){if(this.entities[i].type==ig.Entity.TYPE.B)
...
window.onorientationchange=function(){if(ig.ua.mobile&&(window.orientation==0||window.orientation==180)){document.getElementById('portrait').style.display='block';document.getElementById('game').style.display='none';}
...
ig.FullscreenHelper=new FullscreenHelper();ig.FullscreenHelper.init();ig.main('#canvas',StartScreen,30,480,320,1,ig.ImpactSplashLoader);});